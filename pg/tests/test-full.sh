...
pgpool_result=$(kubectl exec -n "${NAMESPACE}" "${POD_0}" -c postgresql -- \
  psql -h "${pgpool_svc}" -p 9999 -U testuser -d testdb -t -A -c "SELECT 1" 2>/dev/null)
...
assert_contains "pgpool has metrics sidecar" "${pgpool_containers}" "metrics"
...
wait_for_deployment_ready "${NAMESPACE}" "${FULLNAME}-prometheus-exporter" 300
...
exporter_port=$(kubectl get svc -n "${NAMESPACE}" "${FULLNAME}-prometheus-exporter" -o jsonpath='{.spec.ports[0].port}')
assert_eq "exporter service port is 9116" "9116" "${exporter_port}"

# Test: exporter returns metrics
exporter_svc="${FULLNAME}-prometheus-exporter.${NAMESPACE}.svc.cluster.local"
metrics_output=$(kubectl exec -n "${NAMESPACE}" "${POD_0}" -c postgresql -- \
  bash -c "curl -s http://${exporter_svc}:9116/metrics 2>/dev/null | head -20" 2>/dev/null || echo "")
if [[ -n "${metrics_output}" ]]; then
  assert_contains "exporter returns pg metrics" "${metrics_output}" "pg_"
else
  skip "exporter metrics check (curl not available in pod)"
fi
