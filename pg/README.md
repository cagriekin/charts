...
- an `extraVolumes` name may not collide with a chart-managed volume (`data`, `postgresql-config`, `postgresql-tls`, `ext-lib`, `ext-share`, `repmgr-config`, `etcd-tls`, `pg-run`, `pgbackrest-config`) — a `data` collision is silently dropped in favour of the volumeClaimTemplate;
...
Reclaimed as orphans: an agent-minted slot for a **departed** ordinal (no such pod exists), the primary's own slot (it does not stream from itself), and a legacy `repmgr_slot_*` for a departed ordinal — native never streams through one, so it is dead weight, and this is what stops a future repmgr→native migration (#292) from leaving a permanent orphan. Legacy slots are scoped to departed ordinals for the same reason: a live node may still be carrying its stream through one mid-migration. An empty or failed pod list reclaims nothing but self, so a misread can never make every standby's slot look orphaned.
