...
- **pgBackRest PITR restore-validation no longer fails when the primary raises a
  ...
  (e.g. `max_connections=150`), which `validate.sh` strips along with the `include_dir`
  line — so the throwaway instance fell back to initdb defaults (`max_connections=100`) and
  the postmaster died at startup with *"recovery aborted because of insufficient parameter
  settings"*, failing the validation Job (and firing `KubeJobFailed`) even though the backup
  itself was fully restorable. The script now reads the required minimums straight from
