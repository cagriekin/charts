...
- **The startup and readiness probes no longer count the bootstrap's transient postmaster
  ...
  `postgresql.configuration.listen_addresses` without `*`, `localhost` or `127.0.0.1` now
  fails the render: the probes and the HA agent's self-connection both need loopback. Pods
  roll once for the probe change.
