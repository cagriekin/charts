...
func TestMigrateForeignRecoveryConfigCarriesTheUpstreamForward(t *testing.T) {
	...
	// Capture the log: the inherited conninfo comes from postgresql.auto.conf, which repmgr
	// writes WITH the replication password in it, so every line that echoes it has to redact
	// (#298 review). Asserted at the CALL SITES, not just on RedactConninfo -- dropping the
	// wrapper is a one-character edit that the helper's own unit test cannot see, and the repo
	// has had a code-scanning alert for exactly this before.
	var logbuf strings.Builder
	a.log = slog.New(slog.NewTextHandler(&logbuf, nil))
	defer func() {
		if strings.Contains(logbuf.String(), "hunter2") {
			t.Errorf("the replication password reached the log:\n%s", logbuf.String())
		}
		// No positive marker assertion: which lines fire depends on how far the seed gets in this
		// harness, and only some of them echo the conninfo at all. The property that matters is
		// unconditional -- the secret never reaches the log by any path.
		if strings.Contains(logbuf.String(), "password=") &&
			!strings.Contains(logbuf.String(), "password=[redacted]") {
			t.Errorf("a password= appears unredacted in the log:\n%s", logbuf.String())
		}
	}()
	...
	// A postgresql.conf, so writeManagedConf succeeds and the seed reaches the log line that
	// echoes the conninfo -- without it the seed aborts first and the redaction assertion below
	// passes vacuously (verified: dropping RedactConninfo did not fail this test until now).
	if err := os.WriteFile(filepath.Join(dir, "postgresql.conf"), []byte("# test\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	// PodName and a prober, or the slot pre-create is not exercised at all (#298 review):
	// slotNameFor("") is "", which takes the "no parseable pod ordinal" branch and skips the
	// half of this migration that talks to the upstream -- while a.prober being nil there would
	...
	if err := os.WriteFile(auto, []byte(
		"# Do not edit this file manually!\nprimary_conninfo = 'host=pgm-0 port=5432 user=repmgr password=hunter2'\nprimary_slot_name = 'repmgr_slot_2'\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if err := a.migrateForeignRecoveryConfig(context.Background()); err != nil {
	...
	// The other half of the fix, and the one that is NOT visible in the fragment: GenerateConfig
	// rewrites primary_slot_name to pg_ha_slot_2 milliseconds later, so that slot has to exist on
	// the inherited upstream before the postmaster starts, or the walreceiver loops on
...
func TestMigrateForeignRecoveryConfigDoesNotCarryAPrimarysStaleUpstream(t *testing.T) {
	...
	// The worse half of the same mistake: pg_ha_slot_0 pre-created on pgm-1, which is a STANDBY
	// by now, is a permanently inactive slot on a node whose reconcileSlots only runs while it is
	// primary -- the silent WAL pin #289 exists to stop.
	if len(slots.created) != 0 {
		t.Errorf("a primary's migration pre-created slots on its own stale upstream: %v", slots.created)
	}
}
...
// durableTLExec scripts the two queries ensureDurableTimeline makes: the durable/streamed
// timeline read and the CHECKPOINT that repairs it. reads is consumed front to back so the
// re-read after the restartpoint can differ from the one before it.
type durableTLExec struct {
	reads       []string // "durable|streamed" per call, last value repeats
	readCount   int
	checkpoints int
}

func (e *durableTLExec) Run(_ context.Context, _ []string, name string, args ...string) (string, error) {
	sql := args[len(args)-1]
	switch {
	case name == "psql" && strings.Contains(sql, "CHECKPOINT"):
		e.checkpoints++
		return "", nil
	case name == "psql" && strings.Contains(sql, "received_tli"):
		i := min(e.readCount, len(e.reads)-1)
		e.readCount++
		return e.reads[i], nil
	}
	return "", nil
}

func newDurableTLAgent(t *testing.T, ex *durableTLExec) *agent {
	t.Helper()
	return &agent{
		cfg:    &config.Config{PodName: "pgm-1", RepmgrUser: "repmgr", RepmgrDB: "repmgr"},
		log:    slog.New(slog.NewTextHandler(io.Discard, nil)),
		prober: &pg.Prober{Exec: ex, Timeout: time.Second},
		// ensureDurableTimeline heartbeats before the CHECKPOINT, so the harness needs a real
		// metrics registry or the beat nil-derefs (#298 review).
		metr: observe.New(),
	}
}

// #298 review: the comparison is against the timeline the #125 guard would read with no
// walreceiver -- GREATEST(checkpoint TLI, min_recovery_end_timeline) -- so a standby that has
// already recorded the streamed timeline there needs NO restartpoint. Measuring against
// pg_controldata's checkpoint TLI alone called that node stale and forced a full restartpoint on
// it every interval, forever, on a node that was already promotable.
func TestEnsureDurableTimelineDoesNothingWhenTheDurableTimelineIsCurrent(t *testing.T) {
	ex := &durableTLExec{reads: []string{"3|3"}}
	a := newDurableTLAgent(t, ex)
	a.ensureDurableTimeline(context.Background())
	if ex.checkpoints != 0 {
		t.Errorf("forced %d restartpoint(s) on a node whose durable timeline already matches", ex.checkpoints)
	}
}

func TestEnsureDurableTimelineForcesARestartpointWhenItLags(t *testing.T) {
	ex := &durableTLExec{reads: []string{"2|3", "3|3"}}
	a := newDurableTLAgent(t, ex)
	a.ensureDurableTimeline(context.Background())
	if ex.checkpoints != 1 {
		t.Fatalf("want exactly one restartpoint, got %d", ex.checkpoints)
	}
	// Throttled to the normal interval after a repair that landed.
	if got := time.Until(a.durableTLNextAt); got > durableTLInterval || got < durableTLInterval/2 {
		t.Errorf("next check due in %s, want ~%s (not the no-advance backoff)", got, durableTLInterval)
	}
}

// A restartpoint PostgreSQL skipped (no newer checkpoint record replayed yet) must back the check
// off (#298 review). Without it a standby working through a replay backlog behind a timeline
// switch -- or one held back by recovery_min_apply_delay -- takes a forced flush of dirty shared
// buffers every durableTLInterval, indefinitely, for a repair that cannot land until replay
// catches up.
func TestEnsureDurableTimelineBacksOffWhenTheRestartpointDidNotAdvanceIt(t *testing.T) {
	ex := &durableTLExec{reads: []string{"2|3"}} // the re-read still says 2
	a := newDurableTLAgent(t, ex)
	a.ensureDurableTimeline(context.Background())
	if ex.checkpoints != 1 {
		t.Fatalf("want exactly one restartpoint, got %d", ex.checkpoints)
	...
	// And the throttle is honoured: a second call in the same window does nothing at all.
	a.ensureDurableTimeline(context.Background())
	if ex.checkpoints != 1 {
		t.Errorf("the backoff did not hold: %d restartpoints", ex.checkpoints)
	}
}

// durableTLFailExec fails the CHECKPOINT (the shape a restartpoint that overruns
// durableTLBudget takes: psql is cut short, the server-side flush keeps running).
type durableTLFailExec struct {
	durableTLExec
	failCheckpoint bool
	failReread     bool
}

func (e *durableTLFailExec) Run(ctx context.Context, env []string, name string, args ...string) (string, error) {
	sql := args[len(args)-1]
	if name == "psql" && strings.Contains(sql, "CHECKPOINT") && e.failCheckpoint {
		e.checkpoints++
		return "", context.DeadlineExceeded
	}
	if name == "psql" && strings.Contains(sql, "received_tli") && e.failReread && e.readCount > 0 {
		e.readCount++
		return "", context.DeadlineExceeded
	}
	return e.durableTLExec.Run(ctx, env, name, args...)
}

// #298 review: a restartpoint that ERRORS must back off exactly like one that ran and did not
// advance the timeline. The commonest error is durableTLBudget expiring on a genuinely slow
// flush, and cutting psql short does not abort the server-side checkpoint -- so retrying on the
// 15s interval issues a fresh CHECKPOINT at the standby already saturated with replay I/O, which
// is the hammering the backoff exists to prevent.
func TestEnsureDurableTimelineBacksOffWhenTheRestartpointFails(t *testing.T) {
	ex := &durableTLFailExec{durableTLExec: durableTLExec{reads: []string{"2|3"}}, failCheckpoint: true}
	a := newDurableTLAgent(t, &ex.durableTLExec)
	a.prober = &pg.Prober{Exec: ex, Timeout: time.Second}
	a.ensureDurableTimeline(context.Background())
	if ex.checkpoints != 1 {
		t.Fatalf("want exactly one restartpoint attempt, got %d", ex.checkpoints)
	}
	if got := time.Until(a.durableTLNextAt); got <= durableTLInterval {
		t.Errorf("next check due in %s, want the %s backoff", got, durableTLNoAdvanceInterval)
	}
	a.ensureDurableTimeline(context.Background())
	if ex.checkpoints != 1 {
		t.Errorf("the backoff did not hold: %d restartpoint attempts", ex.checkpoints)
	}
}

// Same reasoning for the confirmation read: the restartpoint DID run, and the read most often
// fails because that flush ate the shared budget (#298 review).
func TestEnsureDurableTimelineBacksOffWhenTheConfirmationReadFails(t *testing.T) {
	ex := &durableTLFailExec{durableTLExec: durableTLExec{reads: []string{"2|3"}}, failReread: true}
	a := newDurableTLAgent(t, &ex.durableTLExec)
	a.prober = &pg.Prober{Exec: ex, Timeout: time.Second}
	a.ensureDurableTimeline(context.Background())
	if ex.checkpoints != 1 {
