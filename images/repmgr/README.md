...
1. **Init container** (`init` mode) runs first on each pod and generates `repmgr.conf`.
   The role comes from the CLUSTER and the data directory, not from the ordinal:
   - Empty data directory: probe for a primary first. Only when no peer answers at all is
     this a genuine first boot (exit, and let `postgres` mode `initdb`). If a primary is
     live — or a peer is up but no primary is visible yet — take the standby path and
     clone via `repmgr standby clone` / `pg_basebackup`, whatever the ordinal. An empty
     PVC on ordinal 0 is a PVC-loss recreate, not a first install.
   - Data present with `standby.signal`: standby-state. Take the standby path, which
     skips the clone when the local timeline already matches the primary's.
   - Data present without `standby.signal`: primary-state. Defer the start/rewind
     decision to the entrypoint guard.

2. **Main container** (`postgres` mode):
   - First boot: runs `initdb`, configures WAL replication settings, optionally enables `archive_mode` and `archive_command` for pgBackRest when `PGBACKREST_ENABLED=true`, creates app DB/user and repmgr DB/user/extension, starts postgres
